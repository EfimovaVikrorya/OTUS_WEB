...
for line in output[1:]:
    process_list = line.split(maxsplit=len(headers) - 1)
    user = process_list[headers.index("USER")]
    memory = float(process_list[headers.index("%MEM")])
    cpu = float(process_list[headers.index("%CPU")])
    process = ' '.join(process_list[headers.index("COMMAND")])
